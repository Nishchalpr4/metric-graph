...
cur.execute('''
SELECT filing_id, filing_date, period, company_id
FROM financials_filing
WHERE company_id = %s
AND (
    (EXTRACT(QUARTER FROM filing_date) = 2 AND EXTRACT(YEAR FROM filing_date) = 2024)
    OR
    (EXTRACT(QUARTER FROM filing_date) = 2 AND EXTRACT(YEAR FROM filing_date) = 2021)
)
...
for fid, fdate, period, cid in filings:
    print(f'  Filing ID: {fid}, Date: {fdate}, Period: {period}')
print()
...
for filing_id, filing_date, period, cid in filings:
    cur.execute('''
    ...
    print(f'Filing ID {filing_id} ({filing_date}, Q2 {year}):')
